<div class="operation">
	<?php render_server_ops("processlist"); ?>
</div>
	...
	<tr bgcolor="#fffeee">
		...
		<td valign="top" <?php if(isset($prog["query"])&&$param=="opid"): ?>rowspan="2"<?php endif; ?>>
			<?php if(isset($prog[$param])):?>
				<?php if($param=="opid"):?>
					<?php h($prog["opid"]);?><?php if($prog["opid"]>1):?> [<a href="<?php h(url("killOp", array("opid"=>$prog["opid"]))); ?>" onclick="return window.confirm('<?php hm("killop"); ?> \'<?php h($prog["opid"]);?>\'?')"><?php hm("kill"); ?></a>]<?php endif;?>
				<?php
				else:
